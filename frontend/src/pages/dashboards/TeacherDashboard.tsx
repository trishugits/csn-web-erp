...
import { Users, BookOpen, Calendar, ClipboardCheck, LogOut, User } from 'lucide-react';
...
const TeacherDashboard = () => {
  ...
  const stats = [
    { icon: Users, label: 'Total Students', value: '120', color: 'from-blue-500 to-cyan-500' },
    { icon: BookOpen, label: 'Classes Today', value: '5', color: 'from-green-500 to-emerald-500' },
    { icon: ClipboardCheck, label: 'Pending Assignments', value: '8', color: 'from-orange-500 to-red-500' },
    { icon: Calendar, label: 'Upcoming Events', value: '3', color: 'from-purple-500 to-pink-500' },
  ];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {['Mark Attendance', 'Grade Assignments', 'Post Notice'].map((action, index) => (
              <motion.button
                key={index}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="py-3 px-6 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all"
              >
                {action}
              </motion.button>
            ))}
